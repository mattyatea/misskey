	...
	public async pack<S extends 'MeDetailed' | 'UserDetailedNotMe' | 'UserDetailed' | 'UserLite' = 'UserLite'>(
		...
		const packed = {
			...
			...(isDetailed ? {
				...
				mutualBanners: mutualBanners.length > 0 ? await this.userBannerPiningEntityService.packMany(mutualBanners, me) : [],
				myMutualBanner: myMutualBanner ? await this.userBannerEntityService.pack(myMutualBanner, me) : null,
